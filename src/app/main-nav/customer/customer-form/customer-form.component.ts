...
export class CustomerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id) {
      this.customServ.read(this.id).subscribe(user => {
  ...
  save() {
    ...
    if (this.id) {
      this.customServ.update(this.id, this.data).subscribe(res => {
        this.snackBar.open(`Customer ${this.customerForm.get('firstName').value} has been updated`, '', {
          duration: 3000,
        });
        this.router.navigate(['/admin/customers']);
      });
      ...
      this.customServ.create(this.data).subscribe(res => {
        ...
        this.snackBar.open(`Customer ${this.customerForm.get('firstName').value} has been created`, '', {
          duration: 3000,
        });
        this.router.navigate(['/admin/customers']);
      });
